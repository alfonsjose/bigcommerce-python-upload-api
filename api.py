...
class Connection(object):
    def request(self, method, url, *args):
        ...
        headers = {'Authorization': 'Basic ' + auth, 'User-Agent': None, 'Connection': None,
...
class Brands_Resource(Connection):
    ...
    def get_brand_id(self, product_dict, brands_id_dict ):
        ...
        for brand in brands_id_dict:
            if brand.strip() == product_dict['brand_id'].strip():
...
class OptionSet_Resource(Connection):    
    ...
    def create_option_set(self, name):
        ...
        if response.status_code>400:
            ...
            # self.delete_option_set(option_set_id)
            path = '/option_sets.json'
            option_dict = {'name':name}
            payload = json.dumps(option_dict)           
            response = self.request('POST', path, payload) 
...
class Product_Resource(Connection):
    ...
    def parse_product(self, product_dict):
        ...
        """ Creating a Product"""
        self.product_id = self.create_products(new_product_dict)        
        """Creating Images"""
        img = Image_Resource(product_dict)
        img.create_images(self.product_id)
        return self.product_id, self.option_set_id 
    ...
    def create_products(self, product_dict):
        ...
        if response.status_code>400:
            product_id = self.update_product(product_dict)
        else:
    ...
    def update_product(self, product_dict):
        path = '/products.json?=%s' %product_dict['name']
        response = self.request('GET', path)
    ...
    def create_option_values(self, option_id_dict, values_dict):
        ...
        for values in values_list:
            ...
            if values_dict['label'] not in self.label_list:
                ...
                path = '/options/%s/values.json' %option_id
                xresponse = self.request('GET',path)
    ...
    def create_options(self, option_dict_params, product_name):        
        ...
        for option in options_list:            
            ...
            while int(response.status_code)>400:
                option_dict['display_name'] = option.split('=')[0].split(']')[-1]
                option_dict['name'] = product_name + option_dict['display_name']  + str(self.counter)            
                option_dict['type'] = option.split('=')[0].split(']')[0].strip('[')
                payload = json.dumps(option_dict)
                response = self.request('POST', path, payload)
                print response , response.text
...
class Rules_Resoruce(Connection):
    ...
    def create_rule(self, rules_dict, product_id, sku_id):
    ...
    def get_rule(self, product_id):
        path = '/products/%s/rules.json' %(product_id)
        response = self.request('GET', path)
        filename = 'SKU_ProductID_%s.json' %product_id
        product_json_file = open(filename,'w')
        #
        data = response.json()
        data = json.dumps(data)
...
class CSV_Parse(Connection):
    ...
    def __init__(self, filename):    
        ...
        # c = Category_Resource()
        # self.category_dict = c.get_category_list()
    ...
    def parse_csv(self):
        ...
        # for i in range(64644,64666):
            # options_class.delete_options(i)
 
        for row in csv_list:    
            
                if row['Item Type'] == 'Product':
                    product_id, option_set_id, option_id_dict = None, None, None
                    ...
                    if row['id'] == '':                
                        ...
                        product_id, option_set_id = self.parse_productrow(row, product_class)

                    else:
                        product_id, option_set_id = product_class.get_product_ids(product_name)
                    ...
                    print 'Creating Rule'
                    rules_class.create_rule(row, product_id, sku_id)
...
if __name__ == '__main__':
    ...
    upload = CSV_Parse('update.csv')   
    upload.parse_csv()

    # connection =  Connection()
    # path = '/products.json?name=%s'  %("Harrow Volt Men's White/Black*")
    # response = connection.request('GET', path)
    # for k in response.json():
    #     option_set_id = k['option_set_id']
    #     break
    # print option_set_id 
    ...
    # path = '/option_sets.json?name=%s'  %("Harrow Volt Men's White/Black*")
